...
all_file = listdir('topic/test2/')  # 获取文件夹中所有文件名#数据集地址
outputDir = "topic/test3/"  # 结果输出地址
labels = []  # 用以存储名称
corpus = []  # 空语料库
size = 60  # 测试集容量
...
def buildSW():
    '''停用词的过滤'''
    typetxt = open('STOPW.txt', 'r', encoding='utf-8')  # 停用词文档地址
    texts = ['\u3000', '\n', ' ']  # 爬取的文本中未处理的特殊字符
    '''停用词库的建立'''
    for word in typetxt:
        word = word.strip()
...
def buildWB(texts):
    ...
    for i in range(0, len(all_file)):
        ...
        file_add = 'topic/test2/' + filename  # 数据集地址
        doc = open(file_add, encoding='gb18030').read()
